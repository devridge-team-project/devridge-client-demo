...
import { CommunityWrite } from "connection/api/communityService";
...
export default function CommunityPost() {
  ...
  const onSubmitHandler = async (e: React.ChangeEvent<HTMLFormElement>) => {
    e.preventDefault();
    const status = await CommunityWrite({ title, content, hashtags });
    if (status === 201) {
  ...
  return (
    <Board options={{ gapY: 8 }}>
      <form onSubmit={onSubmitHandler}>
        ...
        <Button title="작성하기" type="submit" options={{ size: "full" }} />
      </form>
    </Board>
  );
}
